...
-- declare countries default array<string> ['PH', 'AR', 'MY', 'ER', 'VN']; --- countries with high trials but low paid
...
with final_table as (
...
), t2p_less_fallback as (
  select
    install_date, network, platform, country
    , trial
    , paid
    , modeled_trial2paid as t2p
  from `ua_transform_dev.trial2paid_less_fallback_model`
), joined_table as (
  select
  ...
  left join t2p_less_fallback lf
    on f.date = lf.install_date
      and f.network = lf.network
      and f.platform = lf.platform
      and f.country = lf.country
), accuracy_table as (
  select
    ...
    , avg(safe_divide((t2p_less_fallback - t2p_actual), t2p_actual)) as less_fallback_mse
  from joined_table
  where
