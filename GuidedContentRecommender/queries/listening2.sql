declare start_date STRING DEFAULT '20250224';
declare end_date STRING DEFAULT '20250224';

SELECT
  event_date, event_name, user_id, user_pseudo_id,
  MAX(CASE WHEN param.key = 'guided_content' THEN param.value.string_value END) AS guided_content,
  MAX(CASE WHEN param.key = 'sounds_selected' THEN param.value.string_value END) AS sounds,
  MAX(CASE WHEN param.key = 'brainwaves_selected' THEN param.value.string_value END) AS brainwaves,
  MAX(CASE WHEN param.key = 'mixes_selected' THEN param.value.string_value END) AS mixes
FROM `relax-melodies-android.analytics_151587246.events_*`,
    unnest(event_params) as param
where
  _table_suffix = start_date and _table_suffix = end_date
  and event_name in ('listening') # , 'toggle_favorite', 'create_favorite_result'
  and (param.key = 'guided_content')
  and (param.value.string_value is not null and param.value.string_value != 'none')
group by
  event_date, event_name, user_id, user_pseudo_id
