...
declare country_list default array<string> ['US', 'CA', 'UK', 'AU', 'FR', 'MX', 'GB'];
declare platform_list default array<string> ['android', 'ios'];
...
select
	...
	avg(trial2paid_internal) as t2p_internal,
	avg(trial2paid_geobydate) as trial2paid_geobydate,
	avg(trial2paid_internal - trial2paid_geobydate) / avg(trial2paid_internal) as t2p_diff,
	-- avg(rev_per_trial_internal - rev_per_trial_geobydate) / avg(rev_per_trial_internal),
	-- avg(rev_per_paid_internal - rev_per_paid_geobydate) / avg(rev_per_paid_internal),
	-- avg(refund_internal - refund_geobydate) / avg(refund_internal),
	-- avg(refunded_amount_internal - refunded_amount_geobydate) / avg(refunded_amount_internal),
from t2p_comparison
where
	network in unnest(network_list)
	and country in unnest(country_list)
	and platform in unnest(platform_list)
	and install_date between start_date and end_date
group by network, platform, country
order by
	platform, network, country;
