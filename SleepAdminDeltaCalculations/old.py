...
from utils import compute_weekly_reach_per_variant, compute_deltas
from utils import compute_absolute_delta_from_relative_delta, compute_sample_needed

def compute_timeline_deltas(monthly_reach: float, conversion_rate: float, initial_timeline: int, power = 0.8, alpha = 0.5, num_weeks = 15, num_variants=2):
    """
    ...
    initial_timeline: int
        Number of weeks required per 'Experiment Sizing Tradeoff' spreadsheet

    """
    # --- compute absolute delta
    weekly_reach = compute_weekly_reach_per_variant(monthly_reach)
    initial_size = int(weekly_reach * initial_timeline)
...
def compute_deltas_timelines(monthly_reach: float, conversion_rate: float, until_relative_delta: float,  power = 0.8, alpha = 0.5, num_variants=2):
    ...
    """
    weekly_reach_per_variant = compute_weekly_reach_per_variant(monthly_reach)
    ...
    for relative_delta in relative_deltas:
        ...
        num_week_needed = float(sample_size_needed / weekly_reach_per_variant)
...
def main():
    ...
    INITIAL_TIMELINE_IN_WEEK = 3

    print(compute_timeline_deltas(monthly_reach=MONTHLY_REACH, conversion_rate=CONVERSION_RATE, intial_timeline = INITIAL_TIMELINE_IN_WEEK))
