...
def convert_images_to_parquet(images_dir: str, output_dir: str, image_extension: str):
    ...
    for img_id in image_ids:
        ...
        if os.path.exists(output_path):  # skip image compression if already converted
            logging.info(f"'{img_id}.parquet' already exists, skiping.")
        ...
        with Image.open(image_path) as img:
            ...
            logging.info(f"Converting image '{img_id}' to parquet")


def convert_subdirectory_images_to_parquet(subdir_path: str, parquet_path: str):
    """
    Given a subdirectory with images (png, jpg), convert all the directory to a single
    parquet file
    """
    pass
...
def upload_files_to_gcs_blob(
    ...
    for img_id in tqdm(image_ids, desc="Uploading"):
        ...
        if bucket.blob(blob_path).exists():  # skip if image is already uploaded
            logging.info("Image {img_id} already uploaded to GCS, skipping.")
...
def run():
    ...
    for subdir in subdirectories:
        convert_images_to_parquet(  # FIXME: turn whole subdir to single parquet file?
    ...
    logging.info("Retrieving Storage Key")
