with rates_ma as (
  select
    ...
    round(avg(`>5-Years`) over (partition by platform order by paid_year_month rows between 2 preceding and current row), 2) as `>5-Years`,
  from `relax-melodies-android.late_conversions.renewal_proceeds_cohorted`
), last_valid_rates as (
  select
    platform,
    ...
    rows between UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as `latest_1Years_renewal_rate`,
    ...
    rows between UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as `latest_2Years_renewal_rate`,
    ...
    rows between UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as `latest_3Years_renewal_rate`,
    ...
    rows between UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as `latest_4Years_renewal_rate`,
    ...
    rows between UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as `latest_5Years_renewal_rate`,
    last_value(`>5-Years`) over (
      partition by platform
      order by case
        when parse_date('%Y-%m', paid_year_month) < date_sub(CURRENT_DATE(), interval 6 YEAR) then paid_year_month end
    rows between UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as `latest_>5Years_renewal_rate`,
  from rates_ma
